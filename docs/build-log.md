...
### 2026-09-05 · Wave 2 · integrator · Four lanes died inside 73 seconds and only one of them told me
What broke            Wave 2 launched H, I, J and K concurrently. Every one of the four stopped writing to its transcript between 21:30:54 and 21:32:07; two of them (H, J) end with `[Request interrupted by user]`. Exactly one produced a task notification — H, as a stream watchdog "no progress for 600s". The other three died silently. Had I trusted the notifications, I would have relaunched one lane and left three dead worktrees running nothing for as long as it took me to notice.
Root cause            Not established, and I will not claim it was. What is on the record: four concurrent lanes, four deaths inside 73 seconds, and a handoff note from Wave 1 saying six concurrent lanes had already exhausted a session cap and killed all six simultaneously. I read that note and launched four anyway, because four is under the strategy's cap of six. The cap was written against a failure I had not yet seen twice.
How we got out        Reconstructed the true state from the filesystem rather than from the notifications: transcript mtimes for all four lanes, `git worktree list` (a dead lane's worktree loses its lock), and `git log main..HEAD` per branch. H had three commits plus uncommitted test scaffolding; I had two commits plus an unrun 588-line evidence parser and its tests; J and K had produced nothing. Salvaged the uncommitted work verbatim as its own commit on each branch, per the Wave 1 rule, removed the four worktrees, and relaunched in two batches of two — H and I first as successors on their existing branches, with briefs naming the inherited commits and telling each successor to audit them as someone else's work and run `make lint && make verify` before writing a line, then J and K.
What now prevents it  Lanes launch two at a time for the rest of this build, which also makes the two-Fable-lane cap fall out for free rather than being remembered. And when any lane stops, check every lane's transcript mtime and worktree lock before acting on the one that notified — a notification is evidence about one agent, not about the wave.
Time lost             ~10 minutes of wall clock, plus everything J and K had done, which was nothing. H and I lost no work at all, because the salvage rule already existed.
